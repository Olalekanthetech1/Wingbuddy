...
export const telegramRuntime = {
  ...
  async start() {
    ...
    if (realTelegramRuntime) {
      ...
      logger.warn("Telegram bot token not configured; waiting for runtime configuration");
    }
  },
  async stop() {
    if (realTelegramRuntime) await realTelegramRuntime.stop();
  },
...
const handleTelegramWebhook = (req: Request, res: Response): void => {
  ...
  if (realTelegramRuntime) {
    ...
    logger.warn({ updateId: update.update_id }, "Webhook received before Telegram runtime was initialized");
  }
};
...
app.get("/api/dashboard/runtime", (_req: Request, res: Response) => {
  ...
  const modelPool = unique([
    primaryModel,
    ...split("GEMINI_MODEL_POOL"),
    ...split("GEMINI_MODEL_FALLBACKS"),
  ]);
  ...
  res.json({
    ...
    primaryModel,
    modelPool,
    fastModel: process.env.GEMINI_MODEL_FAST?.trim() || "",
    reasoningModel: process.env.GEMINI_MODEL_REASONING?.trim() || "",
    extractionModel: process.env.GEMINI_MODEL_EXTRACTION?.trim() || "",
    fallbackModels: split("GEMINI_MODEL_FALLBACKS"),
    embeddingModel: process.env.GEMINI_EMBEDDING_MODEL?.trim() || "",
