...
interface GeminiResponse {
  text?: string;
  candidates?: Array<{ groundingMetadata?: { groundingChunks?: Array<{ web?: { uri?: string; title?: string } }>; webSearchQueries?: string[] } }>;
}
...
export class GeminiService {
  ...
  private getClient(keyInfo?: ManagedKey): GeminiClient {
    ...
    if (!client) {
      client = new GoogleGenAI({ apiKey: rawKey }) as unknown as GeminiClient;
  ...
  private modelCandidates(context: CallContext): string[] {
    return geminiModelPoolService.getCandidates(this.model, {
      mode: context.mode,
      enableSearch: context.enableSearch,
      isDeepReasoning: context.isDeepReasoning,
      isExtraction: context.isExtraction,
    });
  }
  ...
  private buildContents(history: GeminiMessage[], message: string, attachments?: MultimodalAttachment[]): Content[] {
    ...
    return [
      ...
      { role: "user", parts: userParts as unknown as Content["parts"] },
    ];
  }
  ...
  private timeout(context: CallContext): number {
    return this.timeoutMs > 0 ? this.timeoutMs : AdaptiveEngineService.computeAdaptiveTimeout({
      prompt: context.prompt,
      enableSearch: context.enableSearch,
      isDeepReasoning: context.isDeepReasoning,
      hasAudio: context.hasAudio,
      hasVisionOrDocument: context.hasVisionOrDocument,
      mediaSizeBytes: context.mediaSizeBytes,
      targetKeyId: context.targetKeyId,
    });
  }
  ...
  private async callWithTimeout(client: GeminiClient, model: string, contents: Content[], config: Record<string, unknown>, timeoutMs: number): Promise<GeminiResponse> {
    return Promise.race([
      ...
      new Promise<never>((_, reject) => setTimeout(() => reject(new GeminiTimeoutError()), timeoutMs)),
    ]);
  }
    ...
    try {
      for await (const chunk of stream) {
        ...
        for (const c of chunk.candidates?.[0]?.groundingMetadata?.groundingChunks || []) {
          if (c.web?.uri && !sources.some((s) => s.uri === c.web?.uri)) sources.push({ uri: c.web.uri, title: c.web.title });
        }
      }
    } catch (error) {
      // Never retry after partial output; a second model would duplicate streamed content in Telegram.
  ...
  async summarizeSession(messages: GeminiMessage[]): Promise<string> {
    ...
    try {
      return await this.generateReply([], `Summarize this conversation into 2-3 concise factual bullet points focused on user context, problems, and decisions.\n\n${messages.map((m) => `${m.role}: ${m.content}`).join("\n")}`);
    } catch {
      return "";
    }
  }
