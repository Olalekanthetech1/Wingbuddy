export function renderDashboardControlPlane(): string {
  return String.raw`
<style>
.wb-cp{margin-top:16px}.wb-cp-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.wb-cp-stat{padding:11px;border:1px solid var(--line);border-radius:10px;background:#0a1626}.wb-cp-stat strong{display:block;font-size:18px}.wb-cp-stat span{display:block;color:var(--muted);font-size:11px;margin-top:2px}.wb-key-toolbar{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.wb-key-form{display:grid;grid-template-columns:1fr 1.2fr auto;gap:8px;margin-top:10px}.wb-key-card{display:grid;grid-template-columns:1fr auto;gap:10px;padding:12px;border:1px solid var(--line);border-radius:11px;background:#0a1626}.wb-key-meta{display:flex;flex-wrap:wrap;gap:6px;margin-top:5px}.wb-key-actions{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}.wb-cp-note{font-size:12px;color:var(--muted);margin-top:8px}@media(max-width:760px){.wb-cp-grid,.wb-key-form,.wb-key-card{grid-template-columns:1fr}}
</style>
<script>
(function(){
