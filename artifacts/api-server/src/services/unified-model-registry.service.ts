...
export interface UnifiedModelRecord {
  id: string;
  provider: AIProviderId;
  modelId: string;
  name: string;
  roles: UnifiedModelRole[];
  enabled: boolean;
  priority: number;
  capabilities: string[];
  createdAt: string;
...
const makeId = (provider: AIProviderId, modelId: string): string =>
  `${provider}:${modelId}`.replace(/[^a-zA-Z0-9:_-]/g, "_");
...
export class UnifiedModelRegistryService {
  ...
  private enforce(models: UnifiedModelRecord[]): UnifiedModelRecord[] {
    ...
    const sorted = [...deduped.values()].sort((a, b) => a.priority - b.priority || a.provider.localeCompare(b.provider) || a.modelId.localeCompare(b.modelId));
    let primaryFound = false;
    return sorted.map((model) => {
      const roles = model.roles.filter((role) => !(model.roles.includes("embedding") && role === "primary"));
      if (!roles.includes("primary")) return { ...model, roles };
      if (!model.enabled || primaryFound) return { ...model, roles: roles.filter((role) => role !== "primary") };
      primaryFound = true;
      return { ...model, roles };
    });
  }
  ...
  private envBootstrap(): UnifiedModelRecord[] {
    ...
    const sources: Array<{ provider: AIProviderId; role?: UnifiedModelRole; value?: string }> = [
      { provider: "gemini", role: "primary", value: process.env.GEMINI_MODEL?.trim() || process.env.GEMINI_DEFAULT_MODEL?.trim() },
  ...
  private async load(): Promise<UnifiedModelRecord[]> {
    ...
    try {
      ...
      if (rows[0]?.value) {
        this.cache = this.enforce(JSON.parse(rows[0].value));
      ...
      if (legacy[0]?.value) {
        ...
        this.cache = this.enforce(migrated);
        this.cacheAt = Date.now();
        await this.persist(this.cache);
        return this.cache;
      }
    } catch (error) {
      logger.warn({ error: String(error) }, "Failed to read unified AI model registry from PostgreSQL");
    }
    this.cache = this.envBootstrap();
    this.cacheAt = Date.now();
    return this.cache;
  }
  ...
  private async syncLegacyGemini(models: UnifiedModelRecord[]): Promise<void> {
    ...
    const primary = executable.find((model) => model.roles.includes("primary")) || executable[0];
    process.env.GEMINI_MODEL = primary?.modelId || "";
    process.env.GEMINI_MODEL_POOL = executable.map((model) => model.modelId).join(",");
    process.env.GEMINI_MODEL_FALLBACKS = executable.filter((model) => model.id !== primary?.id).map((model) => model.modelId).join(",");
  ...
  private async persist(models: UnifiedModelRecord[]): Promise<void> {
    const normalized = this.enforce(models);
    ...
    await this.syncLegacyGemini(normalized);
    this.cache = normalized;
    this.cacheAt = Date.now();
  }

  async initialize(): Promise<UnifiedModelRecord[]> {
    const models = await this.load();
    await aiProviderRegistryService.list();
    return this.list();
  }

  async list(): Promise<UnifiedModelRecord[]> {
    const models = await this.load();
    return models.map((model) => ({ ...model, roles: [...model.roles], capabilities: [...model.capabilities] }));
  }
  ...
  async add(input: { provider: AIProviderId; modelId: string; name?: string; roles?: UnifiedModelRole[]; priority?: number; capabilities?: string[] }): Promise<UnifiedModelRecord> {
    ...
    const provider = await aiProviderRegistryService.get(input.provider);
    if (!provider.enabled) throw new Error(`Provider ${provider.id} is disabled`);
    const models = await this.list();
    if (models.some((model) => model.provider === input.provider && model.modelId === modelId)) throw new Error(`Model ${input.provider}/${modelId} is already registered.`);
    const roles = unique((input.roles || []).filter((role): role is UnifiedModelRole => ROLES.includes(role)));
    if (roles.includes("primary") && roles.includes("embedding")) throw new Error("Embedding models cannot be primary.");
    if (!models.some((model) => model.enabled && model.roles.includes("primary")) && !roles.includes("embedding")) roles.push("primary");
    const now = new Date().toISOString();
    const model: UnifiedModelRecord = { id: makeId(input.provider, modelId), provider: input.provider, modelId, name: input.name?.trim() || modelId, roles, enabled: true, priority: Number.isFinite(input.priority) ? Number(input.priority) : models.length, capabilities: unique(input.capabilities?.length ? input.capabilities : (roles.includes("embedding") ? ["embedding"] : ["generate"])), createdAt: now, updatedAt: now };
    const next = model.roles.includes("primary") ? models.map((item) => ({ ...item, roles: item.roles.filter((role) => role !== "primary") })).concat(model) : models.concat(model);
  ...
  async update(id: string, patch: Partial<Pick<UnifiedModelRecord, "provider" | "modelId" | "name" | "roles" | "enabled" | "priority" | "capabilities">>): Promise<UnifiedModelRecord> {
    ...
    const modelId = patch.modelId?.trim() || current.modelId;
    const providerRecord = await aiProviderRegistryService.get(provider);
    if (!providerRecord.enabled && provider !== current.provider) throw new Error(`Provider ${provider} is disabled`);
    if (models.some((model, i) => i !== index && model.provider === provider && model.modelId === modelId)) throw new Error(`Model ${provider}/${modelId} is already registered.`);
    const roles = patch.roles ? unique(patch.roles.filter((role): role is UnifiedModelRole => ROLES.includes(role))) : current.roles;
    if (roles.includes("primary") && roles.includes("embedding")) throw new Error("Embedding models cannot be primary.");
    ...
    const updated: UnifiedModelRecord = { ...current, ...patch, provider, modelId, id: makeId(provider, modelId), name: patch.name?.trim() || current.name, roles, enabled: patch.enabled ?? current.enabled, priority: patch.priority ?? current.priority, capabilities: patch.capabilities ? unique(patch.capabilities) : current.capabilities, updatedAt: new Date().toISOString() };
    const base = models.map((model, i) => i === index ? updated : model);
    const normalized = roles.includes("primary") ? base.map((model, i) => i === index ? model : ({ ...model, roles: model.roles.filter((role) => role !== "primary") })) : base;
  ...
  async setPrimary(id: string): Promise<UnifiedModelRecord> {
    ...
    const next = models.map((model) => ({ ...model, roles: model.id === id ? unique([...model.roles.filter((role) => role !== "primary"), "primary"]) : model.roles.filter((role) => role !== "primary") }));
    await this.persist(next);
