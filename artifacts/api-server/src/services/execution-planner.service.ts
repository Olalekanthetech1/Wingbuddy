...
export class ExecutionPlannerService {
  ...
  plan(
    ...
    const semantic = semanticInteractionCache.get(text, persistentMode, history) || this.fallbackDecision(persistentMode);
    const effectiveMode = persistentMode === "auto"
      ? (semantic.effectiveMode || this.modeService.resolveMode("general") || "general")
      : persistentMode;
    ...
    if (semantic.enableSearch && profile.capabilities.toolPermissions.searchAllowed) {
    ...
    const plan: ExecutionPlan = {
      persistentMode,
      effectiveMode,
      turnModeOverride: effectiveMode !== persistentMode ? effectiveMode : undefined,
      detectedIntent,
      requiredCapabilities: Array.from(capabilities),
      enableSearch: semantic.enableSearch,
  ...
  private fallbackDecision(mode: ModeKey): SemanticInteractionDecision {
