...
function jsonOnlyPrompt(
  ...
  return [
    ...
    "Conversational Continuity Rule: Evaluate the immediate preceding assistant turn. If the assistant explicitly requested clarification, a selection, or information needed to continue the conversation, interpret the user's subsequent response in the context of that prompt. Prefer \"NO_TASK\" when the response merely supplies conversational information, clarification, or a selection and does not independently express task-management intent. If the response explicitly identifies, resumes, modifies, creates, cancels, or otherwise acts upon a background task, classify it according to the applicable task-management intent. Do not infer task-management intent solely from the fact that the preceding assistant turn involved a task-related conversation.",
