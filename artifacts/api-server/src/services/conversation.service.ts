...
import { chatDatabaseService, conversationsTable, db, messagesTable, usersTable, type Message, type UserMemoryRecord } from "@workspace/db";
import { isPersonalityKey, type PersonalityKey } from "../config/personality";
import { isModeKey, type ModeKey } from "../config/mode";
import { behaviorRegistryService } from "./behavior-registry.service";

export interface TelegramUserProfile { id: number; username?: string; firstName?: string; lastName?: string; }

export class ConversationService {
  async upsertUser(user: TelegramUserProfile): Promise<void> {
    const now = new Date();
    await db.insert(usersTable).values({ telegramUserId: user.id, username: user.username ?? null, firstName: user.firstName ?? null, lastName: user.lastName ?? null, updatedAt: now }).onConflictDoUpdate({ target: usersTable.telegramUserId, set: { username: user.username ?? null, firstName: user.firstName ?? null, lastName: user.lastName ?? null, updatedAt: now } });
  }
  ...
  async getUserPersonality(telegramUserId: number): Promise<PersonalityKey> {
    ...
    const configured = behaviorRegistryService.getDefault("personality");
    return configured && isPersonalityKey(configured) ? configured : "playful";
  }
  ...
  async getUserMode(telegramUserId: number): Promise<ModeKey> {
    ...
    const configured = behaviorRegistryService.getDefault("mode");
    return configured && isModeKey(configured) ? configured : "auto";
  }
  ...
  private async getConversationId(telegramUserId: number, chatId: number): Promise<number> { const conversation = await db.select({ id: conversationsTable.id }).from(conversationsTable).where(and(eq(conversationsTable.telegramUserId, telegramUserId), eq(conversationsTable.chatId, chatId))).limit(1); if (!conversation[0]) throw new Error("Conversation could not be created."); return conversation[0].id; }
  async addMessage(conversationId: number, role: "user" | "model", content: string): Promise<void> { await db.insert(messagesTable).values({ conversationId, role, content }); await db.update(conversationsTable).set({ updatedAt: new Date() }).where(eq(conversationsTable.id, conversationId)); }
  async getRecentMessages(conversationId: number, limit: number): Promise<Message[]> { const messages = await db.select().from(messagesTable).where(eq(messagesTable.conversationId, conversationId)).orderBy(desc(messagesTable.createdAt)).limit(limit); return messages.reverse(); }
  async clearConversation(telegramUserId: number, chatId: number): Promise<void> { const conversationId = await this.findConversationId(telegramUserId, chatId); if (!conversationId) return; await db.delete(messagesTable).where(eq(messagesTable.conversationId, conversationId)); await db.update(conversationsTable).set({ updatedAt: new Date() }).where(eq(conversationsTable.id, conversationId)); }
  async resetConversation(telegramUserId: number, chatId: number): Promise<void> { const conversationId = await this.findConversationId(telegramUserId, chatId); if (!conversationId) return; await db.delete(messagesTable).where(eq(messagesTable.conversationId, conversationId)); await db.delete(conversationsTable).where(eq(conversationsTable.id, conversationId)); }
  ...
  async getRecentSessionSummary(telegramUserId: number, chatId: number): Promise<string | null> { const session = await db.select({ summary: conversationsTable.summary }).from(conversationsTable).where(and(eq(conversationsTable.telegramUserId, telegramUserId), eq(conversationsTable.chatId, chatId))).limit(1); return session[0]?.summary ?? null; }
  async setSessionSummary(conversationId: number, summary: string): Promise<void> { await db.update(conversationsTable).set({ summary, updatedAt: new Date() }).where(eq(conversationsTable.id, conversationId)); }
  private async findConversationId(telegramUserId: number, chatId: number): Promise<number | undefined> { const result = await db.select({ id: conversationsTable.id }).from(conversationsTable).where(and(eq(conversationsTable.telegramUserId, telegramUserId), eq(conversationsTable.chatId, chatId))).limit(1); return result[0]?.id; }
}
