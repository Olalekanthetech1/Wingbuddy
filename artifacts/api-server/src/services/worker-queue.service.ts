...
export class TelegramWorkerQueueService {
  ...
  /**
   * Sets the handler callback (typically `bot.handleUpdate`) that processes updates asynchronously.
   */
  ...
  /**
   * Dynamically computes max worker concurrency based on active healthy API keys.
   * If 1 key is healthy: 3 concurrent workers.
   * If 3 keys are healthy: 9 concurrent workers.
   */
  computeAdaptiveConcurrency(): number {
    try {
      ...
      // Base concurrency per healthy key is 3, scaled down if keys are cooling down
  ...
  /**
   * Enqueues an incoming Telegram update.
   * Returns immediately so HTTP webhook handler can respond 200 OK in ~2ms.
   */
  enqueue(update: Update): { taskId: string; queueLength: number } {
    ...
    const queue = this.userQueues.get(userOrChatKey)!;
    queue.push(task);
  ...
  /**
   * Extracts a per-user routing key (e.g. `user_12345` or `chat_67890`) to guarantee
   * strict FIFO message ordering for each user conversation without blocking other users.
   */
  ...
  private scheduleProcessing(): void {
    ...
    for (const [userKey, queue] of this.userQueues.entries()) {
      ...
      if (this.processingUsers.has(userKey)) continue; // Keep strict FIFO per user
      if (queue.length === 0) {
      ...
      logger.debug(
        {
          taskId: task.id,
          userKey,
          updateId: task.update.update_id,
          durationMs: duration,
          waitTimeMs: start - task.receivedAt,
        },
        "Worker completed Telegram update task",
      );
    } catch (error) {
      this.failedTotal += 1;
      logger.error(
        ...
        "Worker failed executing Telegram update task",
      );
    } finally {
      this.activeWorkers = Math.max(0, this.activeWorkers - 1);
      this.processingUsers.delete(userKey);

      // Clean up empty queue bucket
      const currentQueue = this.userQueues.get(userKey);
      if (currentQueue && currentQueue.length === 0) {
        this.userQueues.delete(userKey);
      }

      // Schedule next available task
  ...
  getMetrics(): QueueMetrics {
    const avgProcessingTimeMs =
      this.completedTotal > 0
        ? Math.round(this.totalProcessingTimeMs / this.completedTotal)
        : 0;
    ...
    return {
      ...
      maxConcurrency: this.computeAdaptiveConcurrency(),
      pendingQueueLength: this.getPendingCount(),
      avgProcessingTimeMs,
  ...
  async drain(timeoutMs = 5000): Promise<void> {
    ...
    while (
      (this.activeWorkers > 0 || this.getPendingCount() > 0) &&
      Date.now() < deadline
    ) {
