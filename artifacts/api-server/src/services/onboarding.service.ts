...
function normalizeStep(value: unknown): OnboardingStep {
  const allowed: OnboardingStep[] = ["welcome", "personality", "mode", "proactivity", "memory", "about_you", "timezone", "ready"];
  return typeof value === "string" && allowed.includes(value as OnboardingStep) ? value as OnboardingStep : "welcome";
}
...
export class OnboardingService {
  ...
  async get(telegramUserId: number): Promise<OnboardingState | null> {
    try {
      ...
      logger.warn({ telegramUserId, error: error instanceof Error ? error.message : String(error) }, "Failed to read onboarding state");
      throw error;
    }
  }
  ...
  async save(
    ...
    const nextStep = normalizeStep(patch.step ?? current?.step ?? "welcome");
    const nextProactivity = normalizeProactivity(patch.proactivityPreference ?? current?.proactivityPreference);
    const nextMemory = patch.memoryEnabled ?? current?.memoryEnabled ?? true;
    const nextTimezone = patch.timezone?.trim() || current?.timezone || "Africa/Lagos";
    const completed = nextStatus === "completed" ? new Date() : null;

    await getPool().query(`
      INSERT INTO ${TABLE}(
        telegram_user_id, chat_id, status, step, proactivity_preference, memory_enabled, timezone, updated_at, completed_at
    ...
    `, [telegramUserId, chatId, nextStatus, nextStep, nextProactivity, nextMemory, nextTimezone, completed]);
