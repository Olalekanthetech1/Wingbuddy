...
function requireToken(token?: string): string { const value = token?.trim() || process.env.HF_TOKEN?.trim(); if (!value) throw new Error("HF_TOKEN is not configured on the server"); return value; }
...
export class HuggingFaceMediaService {
  ...
  async generateImage(prompt:string,options:{model?:string;width?:number;height?:number}={}):Promise<HuggingFaceMediaResult>{
    const client=new InferenceClient(requireToken()); const model=await this.resolveRegisteredModel("text-to-image",options.model); const controller=new AbortController(); const timeout=setTimeout(()=>controller.abort(),120_000);
  ...
  async generateVideo(prompt:string,options:{model?:string}={}):Promise<HuggingFaceMediaResult>{
    const client=new InferenceClient(requireToken()); const model=await this.resolveRegisteredModel("text-to-video",options.model); const controller=new AbortController(); const timeout=setTimeout(()=>controller.abort(),240_000);
  ...
  async testModel(model:{modelId:string;capabilities:string[]}):Promise<HuggingFaceMediaTestResult>{
    ...
    const started=Date.now(); try{const result=task==="text-to-image"?await this.generateImage("production smoke test: a simple red apple on a clean white background",{model:model.modelId,width:512,height:512}):await this.generateVideo("production smoke test: a red ball rolling slowly across a clean white floor",{model:model.modelId}); return {ok:true,provider:"huggingface",model:model.modelId,task,latencyMs:Date.now()-started,mimeType:result.mimeType,sizeBytes:result.buffer.length};}catch(error){logger.warn({model:model.modelId,task,error:errorMessage(error)},"Hugging Face media model smoke test failed");return {ok:false,provider:"huggingface",model:model.modelId,task,latencyMs:Date.now()-started,error:errorMessage(error)};}
  }
}
