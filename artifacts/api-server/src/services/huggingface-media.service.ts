...
async function resolveRuntimeToken(): Promise<string> { try { await aiProviderKeyPoolService.hydrateProvider("huggingface", "HF_TOKEN"); } catch (error) { logger.warn({ error: error instanceof Error ? error.message : String(error) }, "Unable to hydrate managed Hugging Face tokens; checking bootstrap token"); } const managed = aiProviderKeyPoolService.getOrderedKeys("huggingface"); if (managed.length) return managed[0].key; return requireToken(); }
...
export class HuggingFaceMediaService {
  ...
  async generateImage(prompt:string,options:{model?:string;width?:number;height?:number}={}):Promise<HuggingFaceMediaResult>{
    const token=await resolveRuntimeToken(); const client=new InferenceClient(token); const model=await this.resolveRegisteredModel("text-to-image",options.model); const controller=new AbortController(); const timeout=setTimeout(()=>controller.abort(),120_000);
  ...
  async generateVideo(prompt:string,options:{model?:string}={}):Promise<HuggingFaceMediaResult>{
    const token=await resolveRuntimeToken(); const client=new InferenceClient(token); const model=await this.resolveRegisteredModel("text-to-video",options.model); const controller=new AbortController(); const timeout=setTimeout(()=>controller.abort(),240_000);
  ...
  async testModelWithKey(model:{modelId:string;capabilities:string[]},token:string):Promise<HuggingFaceMediaTestResult>{
    ...
    const started=Date.now(); try{const client=new InferenceClient(requireToken(token)); const result=task==="text-to-image"?await this.generateImageWithToken("production smoke test: a simple red apple on a clean white background",token,{model:model.modelId,width:512,height:512}):await this.generateVideoWithToken("production smoke test: a red ball rolling slowly across a clean white floor",token,{model:model.modelId}); return {ok:true,provider:"huggingface",model:model.modelId,task,latencyMs:Date.now()-started,mimeType:result.mimeType,sizeBytes:result.buffer.length};}catch(error){logger.warn({model:model.modelId,task,error:errorMessage(error)},"Hugging Face media model smoke test failed");return {ok:false,provider:"huggingface",model:model.modelId,task,latencyMs:Date.now()-started,error:errorMessage(error)};}
  }

  async testModel(model:{modelId:string;capabilities:string[]}):Promise<HuggingFaceMediaTestResult>{ return this.testModelWithKey(model, await resolveRuntimeToken()); }

  private async generateImageWithToken(prompt:string,token:string,options:{model:string;width?:number;height?:number}={}):Promise<HuggingFaceMediaResult>{ const client=new InferenceClient(requireToken(token)); const model=await this.resolveRegisteredModel("text-to-image",options.model); const controller=new AbortController(); const timeout=setTimeout(()=>controller.abort(),120_000); try{const parameters:Record<string,unknown>={}; if(Number.isFinite(options.width)) parameters.width=options.width; if(Number.isFinite(options.height)) parameters.height=options.height; const blob=await client.textToImage({model,inputs:prompt,provider:"auto",...(Object.keys(parameters).length?{parameters}: {})},{signal:controller.signal,retry_on_error:true}); const buffer=Buffer.from(await blob.arrayBuffer()); const mimeType=detectImageMime(buffer); if(!mimeType||buffer.length<2000) throw new Error("Hugging Face returned an invalid image payload"); return {buffer,mimeType,model,provider:"huggingface",task:"text-to-image"};} finally{clearTimeout(timeout);} }
  private async generateVideoWithToken(prompt:string,token:string,options:{model:string}={}):Promise<HuggingFaceMediaResult>{ const client=new InferenceClient(requireToken(token)); const model=await this.resolveRegisteredModel("text-to-video",options.model); const controller=new AbortController(); const timeout=setTimeout(()=>controller.abort(),240_000); try{const blob=await client.textToVideo({model,inputs:prompt,provider:"auto"},{signal:controller.signal,retry_on_error:true}); const buffer=Buffer.from(await blob.arrayBuffer()); const mimeType=detectVideoMime(buffer); if(!mimeType||buffer.length<2000) throw new Error("Hugging Face returned an invalid video payload"); return {buffer,mimeType,model,provider:"huggingface",task:"text-to-video"};} finally{clearTimeout(timeout);} }
}
