...
function normalize(value: unknown): AIProviderRecord[] {
  ...
  return value.filter((item): item is AIProviderRecord => {
    ...
    return {
      ...
      capabilities: Array.isArray(item.capabilities) ? item.capabilities.filter((value): value is AIProviderCapability => defaults.capabilities.includes(value)) : [...defaults.capabilities],
...
export class AIProviderRegistryService {
  ...
  private async read(): Promise<AIProviderRecord[]> {
    ...
    try {
      ...
      if (rows[0]?.value) {
        ...
        const merged = Object.values(BUILT_IN_PROVIDERS).map((defaults) => byId.get(defaults.id) || { ...defaults, enabled: false, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
  ...
  async update(id: string, patch: Partial<Pick<AIProviderRecord, "name" | "enabled" | "baseUrl" | "capabilities">>): Promise<AIProviderRecord> {
    const providers = await this.read(); const index = providers.findIndex((provider) => provider.id === id); if (index < 0) throw new Error(`Unknown AI provider: ${id}`);
    const current = providers[index]; const next: AIProviderRecord = { ...current, ...(typeof patch.name === "string" && patch.name.trim() ? { name: patch.name.trim() } : {}), ...(typeof patch.enabled === "boolean" ? { enabled: patch.enabled } : {}), ...(typeof patch.baseUrl === "string" && patch.baseUrl.trim() ? { baseUrl: patch.baseUrl.trim().replace(/\/+$/, "") } : {}), ...(Array.isArray(patch.capabilities) ? { capabilities: patch.capabilities } : {}), updatedAt: new Date().toISOString() };
    await this.persist(providers.map((provider, i) => i === index ? next : provider)); return next;
  }

  async test(id: string, model: string) {
    const provider = await this.get(id); if (!provider.enabled) throw new Error(`Provider ${id} is disabled`);
    await aiProviderKeyPoolService.hydrateProvider(provider.id, provider.apiKeyEnv);
    const adapter = aiProviderAdapters[provider.adapter]; if (!adapter) throw new Error(`No adapter is registered for provider ${id}`);
    const keys = aiProviderKeyPoolService.getOrderedKeys(provider.id, provider.apiKeyEnv);
    if (!keys.length) return adapter.test(model.trim(), provider);
    const key = keys[0]; const result = await adapter.test(model.trim(), provider, key.key); if (result.ok) aiProviderKeyPoolService.recordSuccess(key.id, result.latencyMs); else aiProviderKeyPoolService.recordFailure(key.id, result.error || "Provider test failed"); return result;
  }
