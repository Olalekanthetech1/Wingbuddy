...
/**
 * Authoritative production tools.
 *
 * The planner may propose actions, but these registry definitions remain the
 * source of truth for side effects, approval, capabilities, retry safety and
 * timeouts.
 */
...
function parseDueAt(value: unknown): Date {
  ...
  if (Number.isNaN(dueAt.getTime())) {
    throw new Error(`Invalid dueAt timestamp: ${raw}`);
  }
...
// 1. Deterministic arithmetic calculator
...
// 2. Real external informational lookup via Wikipedia's public REST API.
// This deliberately does not fabricate catalogued findings when no remote result exists.
  ...
  execute: async (input: unknown, context) => {
    ...
    try {
      const response = await fetch(url, {
        method: "GET",
        headers: { "Accept": "application/json", "User-Agent": "Wingbuddy/1.0" },
        signal: controller.signal,
      });

      if (!response.ok) {
        throw new Error(`External lookup failed with HTTP ${response.status}.`);
      }
      ...
      if (!extract) {
        throw new Error("External lookup returned no substantive finding.");
      }
      ...
      return {
        ...
        findings: [extract],
        sourceUrl: payload.content_urls?.desktop?.page || url,
        retrievedAt: new Date().toISOString(),
        requestedByUserId: context.telegramUserId,
      };
    } finally {
...
// 3. Deterministic text condensation
...
// 5. Durable task creation. The action is explicitly requested by the user through the
// execution plan, so confirmation is not needed; retries are disabled by non-idempotent policy.
export const createTaskTool: AssistantTool = {
  ...
  execute: async (input: unknown, context) => {
    ...
    const steps = stepsInput.map((step, index) => {
      ...
      return {
        title: requireNonEmptyString(item.title, `steps[${index}].title`),
    ...
    const task = await taskService.createTask({
      ...
      status: ["pending", "active", "paused", "waiting"].includes(status) ? status as any : "active",
      steps,
      contextData: typeof record.contextData === "object" && record.contextData !== null
        ? record.contextData as Record<string, unknown>
        : undefined,
      metadataData: {
...
// 6. Durable one-time reminder creation.
...
// 7. Destructive administrative session wiper.
