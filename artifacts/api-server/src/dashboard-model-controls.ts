...
(function(){
  ...
  function modelSupportedRoles(model){
    if(!model||!Array.isArray(model.capabilities))return [];
    const capabilities=new Set(model.capabilities.map(v=>String(v).trim().toLowerCase()).filter(Boolean));
    return ROUTING_ROLES.filter(role=>capabilities.has(role));
  }
  function renderRoleSelector(savedRoles){
    const container=document.getElementById('wbModelRoles');
    if(!container)return;
    const model=selectedCatalogModel();
    const supported=modelSupportedRoles(model);
    const wanted=new Set(Array.isArray(savedRoles)?savedRoles:[]);
    if(!model){container.innerHTML='<span class="wb-role-note">Select a model from the live catalog to see the routing roles it supports.</span>';return}
    if(!supported.length){container.innerHTML='<span class="wb-role-note">This model reports no Fast, Reasoning, Extraction, or Embedding capability.</span>';return}
    container.innerHTML=supported.map(role=>'<label class="wb-role"><input type="checkbox" value="'+esc(role)+'"'+(wanted.has(role)?' checked':'')+'> '+esc(role)+'</label>').join('');
  }
  function setForm(model){
    ...
    renderRoleSelector(model?.roles||[]);
    updateCatalogMeta();
  ...
  function populateModelSelect(provider,selectedId,savedRoles=[]){
  ...
  async function saveModel(){
    ...
    if(!selected){toast('That model is not present in the current provider catalog');return}
    if(editingId){try{await api('/api/models/'+encodeURIComponent(editingId),{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider,modelId,name,priority:Number.isFinite(priority)?priority:undefined,roles})});toast('✓ Model updated and saved to PostgreSQL');editingId=null;setForm(null);await loadModelRegistry()}catch(e){toast('Model update failed: '+e.message)}return}
    try{const d=await api('/api/models',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider,modelId,name,priority:Number.isFinite(priority)?priority:undefined,roles,capabilities:selected.capabilities})});setForm(null);toast('✓ '+(d.message||'Model registered'));await loadModelRegistry()}catch(e){toast('Model registration failed: '+e.message)}
  }
  async function beginEdit(id){const model=cachedModels.find(m=>m.id===id);if(!model)return;editingId=id;setForm(model);document.getElementById('wbModelEditor').scrollIntoView({behavior:'smooth',block:'nearest'});await loadCatalog(model.provider,false,model.roles||[])}
