...
function resolveCapability(value: unknown): MediaCapability {
  ...
  if (!MEDIA_CAPABILITIES.has(capability)) {
    throw new Error("Media capability must be image_generation or video_generation");
  }
...
router.get("/media-models/catalog/:provider", async (req: Request, res: Response) => {
  try {
    ...
    let models = [] as Awaited<ReturnType<typeof aiModelCatalogService.list>>;
    if (provider === "huggingface") {
      ...
      try {
        ...
        models = await aiModelCatalogService.list(provider, req.query.refresh === "true");
      }
    } else {
      models = await aiModelCatalogService.list(provider, req.query.refresh === "true");
    }

    const filtered = models.filter((model) => model.capabilities.includes(capability));
    ...
    res.json({
      ...
      capability,
      timestamp: new Date().toISOString(),
      source: provider === "huggingface" ? "huggingface_capability_discovery" : "provider_api",
      models: filtered,
      registeredModelIds: [...registeredIds],
    });
  } catch (error) {
    res.status(400).json({ error: error instanceof Error ? error.message : String(error) });
  }
...
router.post("/media-models", async (req: Request, res: Response) => {
  try {
    ...
    const selected = catalog.find((model) => model.modelId === modelId && model.capabilities.includes(capability));
    if (!selected) {
      if (provider === "huggingface") {
        const task = capability === "image_generation" ? "text-to-image" : "text-to-video";
        const discovery = await huggingFaceCapabilityService.resolveModel(task, modelId);
        if (!discovery.candidates.some((model) => model.id === modelId)) {
          throw new Error(`${provider}/${modelId} is not currently discoverable for ${capability}`);
        }
      } else {
        throw new Error(`${provider}/${modelId} is not currently available for ${capability}`);
      }
    }
    ...
    const model = await unifiedModelRegistryService.add({
      provider,
      modelId,
      name: name || selected?.name || modelId,
      priority,
      capabilities: [capability],
      roles: [],
    });
...
router.patch("/media-models/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (!current) throw new Error("Media model not found");
    const mediaCapability = current.capabilities.find((item) => MEDIA_CAPABILITIES.has(item));
    if (!mediaCapability) throw new Error("Selected model is not a registered media model");
...
router.post("/media-models/:id/test", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const prompt = capability === "image_generation"
      ? "A simple studio test image of a red geometric cube on a clean background"
      : "A simple cinematic test clip of a red ball moving across a clean studio floor";
