...
describe("ApiKeyPoolService", () => {
  ...
  it("initializes with multiple keys from environment variables and formats summary correctly", () => {
    ...
    const summary = new ApiKeyPoolService().getSummary();
  ...
  it("allows toggling keys between enabled and disabled states", () => {
    ...
    const pool = new ApiKeyPoolService();
    const keyId = pool.getSummary().keys[0].id;
  ...
  it("successfully adds a key in cooldown status when the key receives a 429 quota error", async () => {
    ...
    try {
      const summary = pool.getSummary();
      for (const k of summary.keys) if (k.name.includes("Rate Limited") || k.name.includes("Test")) await pool.removeKey(k.id);
    } catch {}
    vi.spyOn(pool, "testRawKey").mockResolvedValue({ valid: true, isRateLimited: true, retryDelaySeconds: 40, latencyMs: 120, notice: "Key is authenticated with Gemini, but currently in rate-limit quota cooldown (~40s)." });
